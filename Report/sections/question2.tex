...
\partquestion{a)} Batch gradient descent update rule:
\[w^{(k+1)} = w^{(k)} - \eta \nabla L(w^{(k)}),\]
...
\partquestion{b)} For the squared loss $\ell(w;x_i;y_i) = (w^{T}x_i - y_i)^2$, the gradient for a single example ($x_i,y_i$) is:
\[\nabla_w\ell(w;x_i;y_i) = 2x_i(w^{T}x_i - y_i)\]
Thus, the update rule for squared loss is:
\[w^{(k+1)} = w^{(k)} - 2\eta x_i({w^{(k)}}^{T}x_i - y_i)\]

\partquestion{c)} When the learning rate is too large, the updates will bounce around the minimum, 
...
\begin{partquestion2}{a)} Machine learning algorithms and in this case a logistic regression model using SGD do no 
...
\partquestion{b)} The logistic loss for a single example of ($x_i,y_i$) where $\hat{p_i} = \sigma(w^Tx_i)$ and
$\sigma(z) = 1/(1+e^{-z})$ is:
...
\begin{partquestion2}{c)} Yes it can and should be used in this particular churn dataset. The churn dataset is
